...
def left_bin_search(array, item, start):
    if not start:
        start = -1
    end = len(array)
...
def right_intersection(array1, array2):
...
def left_intersection(array1, array2):
    if len(array1) > len(array2):
        tmp = array1
        array1 = array2
        array2 = tmp
    pre_item, found_index, last_found_index = None, None, None
    for item in array1:
        if pre_item != item:
            found_index = left_bin_search(array2, item, last_found_index)
            last_found_index = found_index
            if found_index >= 0:
                yield item
        elif found_index < len(array2):
            if array2[found_index + 1] == item:
                yield item
            else:
                found_index = None
